...
=====================
NAME: reatask
PURPOSE: Task manager for Reaper, the Digital Audio Workstation
LANGUAGE: reascript
VERSION: 2.0
DESCRIPTION:
    ...
    if proj_id ~= current_project_path then
        ...
        if r.file_exists(data_file) then
            ...
            if file then
                ...
                for line in content:gmatch("[^\r\n]+") do
                    local id, parent_id, text, done, order, collapsed = line:match("^(%d+):([^:]*):([^:]*):(%w+):(%d+):([^:]*)")
                ...
                table.sort(root_list, function(a, b) return a.order < b.order end)
                root_items = {}
...
function save_todo_data()
    ...
    if file then
        for _, item in pairs(todo_items) do
            file:write(string.format("%d:%s:%s:%s:%d:%s\n", 
                item.id,
...
function draw_menu()
    ...
    for i, menu_item in ipairs(menu_items) do
        ...
        local mouse_over = gfx.mouse_x >= adj_x and gfx.mouse_x <= adj_x + menu_width and
                          gfx.mouse_y >= item_y and gfx.mouse_y <= item_y + menu_item_height
        ...
        if (menu_item.text == "Sort Custom" and sort_mode == "custom") or
           (menu_item.text == "Sort Alphabetical" and sort_mode == "alphabetical") then
            text_color = 1.0
        ...
        for _, display_item in ipairs(display_list) do
            if display_item.id == drop_target.id then
                if drop_target.position == "before" or drop_target.position == "after" then
                    ...
                    gfx.set(1, 0.5, 0, 0.8)
                    gfx.rect(10 + (display_item.level * indent_size), temp_y, 
                            gfx.w - 20 - (display_item.level * indent_size), item_height)
                end
                break
            end
    ...
    for i, display_item in ipairs(display_list) do
        ...
        if y > -item_height and y < gfx.h - input_height and display_item.id ~= drag_item_id then
            ...
            if item.done then
                ...
                gfx.rect(x + inner_margin, y + checkbox_y_offset + inner_margin, 
                        checkbox_size - inner_margin * 2, checkbox_size - inner_margin * 2)
            end
...
function handle_mouse()
    ...
    if menu_open and mouse_cap & 1 == 1 then
        ...
        local clicking_menu = mouse_x >= adj_x and mouse_x <= adj_x + menu_width and
                             mouse_y >= adj_y and mouse_y <= adj_y + menu_height
    ...
    if mouse_cap & 1 == 1 then -- Left mouse down
        if not mouse_down_time or mouse_down_time == 0 then
            ...
            if not dragging and mouse_down_item and 
               (hold_time > click_threshold_time or move_distance > move_threshold) then
                -- Start dragging
                dragging = true
...
function is_window_focused()
    if is_docked() then
        ...
        return gfx.mouse_x >= 0 and gfx.mouse_x <= gfx.w and 
